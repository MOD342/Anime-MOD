...
async function translateToArabicFree(text: string): Promise<string> {
  try {
    ...
    if (response.ok) {
      ...
      if (Array.isArray(json) && Array.isArray(json[0])) {
        const translatedText = json[0].map((item: any) => item[0]).join('').trim();
        if (translatedText) return translatedText;
      }
    }
  } catch (err: any) {
    console.warn("[FreeTranslation] Google Translate backend failed:", err.message);
  }
  return text;
}
...
async function translateToArabic(text: string): Promise<string> {
  ...
  if (client) {
    ...
    for (const model of modelsToTry) {
      try {
        ...
        console.warn(
          `Translation using ${model} failed:`,
          error.message || error,
        );
      }
    }
  ...
  // If Gemini failed or is not available, try the free translation fallback!
  if (!translatedText) {
    console.log(
      "[Translator] Gemini translation was unavailable or failed. Using free fallback...",
    );
    translatedText = await translateToArabicFree(text);
  }

  if (translatedText && translatedText !== text) {
    try {
      await serverCache.set<string>(
        cacheKey,
        translatedText,
        30 * 24 * 60 * 60 * 1000,
      ); // 30 days cache for ultra speed
    } catch (cacheErr) {}
...
async function getArabicTitle(enTitle: string): Promise<string> {
  try {
    if (!enTitle) return "";
    const lower = enTitle.toLowerCase().trim();

    // 1. Check local reverse map
    for (const [ar, en] of Object.entries(LOCAL_ANIME_MAP)) {
      if (en.toLowerCase().trim() === lower) {
        return ar;
      }
    }

    // 2. Try server cache
    const cacheKey = `title_ar_${crypto.createHash("md5").update(lower).digest("hex")}`;
    try {
      const cached = await serverCache.get<string>(cacheKey);
      if (cached) return cached;
    } catch {}

    // 3. Fast Google Translate in background to prevent API blocking!
    // Return English title immediately so pages load instantly (under 100ms).
    // The translation will be fetched in background and cached for the subsequent requests.
    translateAndCacheInBackground(enTitle, cacheKey);
  } catch (err: any) {
    console.warn(`[getArabicTitle] failed for "${enTitle}":`, err.message);
  }
  return enTitle;
}

function translateAndCacheInBackground(enTitle: string, cacheKey: string) {
  // Fire and forget so we never block client response time under rate-limiting
  translateToArabicFree(enTitle).then((translated) => {
    if (translated && translated !== enTitle) {
      serverCache.set<string>(cacheKey, translated, 30 * 24 * 60 * 60 * 1000).catch(() => {});
    }
  }).catch((err: any) => {
    console.warn(`[BackgroundTranslation] Failed for ${enTitle}:`, err.message);
  });
}
...
async function translateTitleToEnglish(title: string): Promise<string> {
  try {
    ...
    // Try server cache
    const cacheKey = `title_en_${crypto.createHash("md5").update(title.toLowerCase().trim()).digest("hex")}`;
    try {
      const cached = await serverCache.get<string>(cacheKey);
      if (cached) return cached;
    } catch {}

    // Use free Google Translate to translate Arabic title to English
    try {
      const controller = new AbortController();
      const timeoutId = setTimeout(() => controller.abort(), 1200);
      const response = await fetch(
        `https://translate.googleapis.com/translate_a/single?client=gtx&sl=ar&tl=en&dt=t&q=${encodeURIComponent(title)}`,
        { signal: controller.signal }
      );
      clearTimeout(timeoutId);
      if (response.ok) {
        ...
        if (Array.isArray(json) && Array.isArray(json[0])) {
          const translatedText = json[0].map((item: any) => item[0]).join('').trim();
          if (translatedText && translatedText !== title) {
            try {
              await serverCache.set<string>(cacheKey, translatedText, 30 * 24 * 60 * 60 * 1000);
            } catch {}
            return translatedText;
          }
        }
      ...
      console.warn("[FreeTranslation] Google Translate to English failed or timed out:", err.message);
    }
...
function getStaticFallback(url: string, error?: any): any {
  const safeMessage = error?.message
    ? String(error.message).replace(/error/gi, "status-alert")
    : "None";
  console.log(
    `[Jikan Fallback Info] Fetch resolved with static fallback for "${url}". Details:`,
    safeMessage,
  );
      ...
      data: [
        ...
        },
        {
          ...
          images: {
            jpg: {
              large_image_url:
                "https://cdn.myanimelist.net/images/anime/1120/120495l.jpg",
            },
          },
          score: 8.6,
        },
        {
          ...
          images: {
            jpg: {
              large_image_url:
                "https://cdn.myanimelist.net/images/anime/1337/138659l.jpg",
            },
          },
          score: 9.04,
        },
        ...
        },
        {
          ...
          images: {
            jpg: {
              large_image_url:
                "https://cdn.myanimelist.net/images/anime/1015/138075l.jpg",
            },
          },
          score: 9.39,
        },
      ...
      data: [
        ...
        },
        {
          ...
          images: {
            jpg: {
              large_image_url:
                "https://cdn.myanimelist.net/images/anime/1097/140263l.jpg",
            },
          },
          score: 8.35,
      ...
      data: [
        ...
        },
        {
          ...
          images: {
            jpg: {
              large_image_url:
                "https://cdn.myanimelist.net/images/anime/1039/141887l.jpg",
            },
          },
          broadcast: {
            time: "23:00",
  ...
  if (/\/anime\/\d+$/.test(url)) {
    ...
    return {
      data: {
        ...
        synopsis:
          "This information is currently loaded from our static offline database due to Jikan API gateway unavailability. Watch links, discussions, and stream functionality remain active.",
        images: {
          jpg: {
            large_image_url:
              "https://cdn.myanimelist.net/images/anime/1244/138851l.jpg",
    ...
    if (staleData !== null) {
      if (!activeJikanLocks.has(url)) {
        const bgPromise = performActualJikanFetch(url, retries, useCache)
          ...
          .catch((err) => {
            activeJikanLocks.delete(url);
            const safeAlert = err?.message
              ? String(err.message).replace(/error/gi, "status-alert")
              : "None";
...
async function performActualJikanFetch(
  url: string,
  retries = 2,
  useCache = true,
): Promise<any> {
  ...
  for (let i = 0; i < retries; i++) {
    ...
    try {
      ...
      if (i === retries - 1) {
        ...
        if (fallback !== null) {
          console.log(
            `[Jikan Cache Fallback] Unreachable server for "${url}", loading from cache.`,
          );
          return fallback;
        }
  ...
  return getStaticFallback(
    url,
    new Error("Jikan API rate limit alert after retries"),
  );
}
...
function unpackDeanEdwards(packed: string): string {
  try {
    const regex =
      /eval\s*\(\s*function\s*\(\s*p\s*,\s*a\s*,\s*c\s*,\s*k\s*,\s*e\s*,[\s\S]*?\}\s*\(\s*(['"][\s\S]+?['"])\s*,\s*(\d+)\s*,\s*(\d+)\s*,\s*(['"][\s\S]+?['"])\.split/i;
    const match = packed.match(regex);
    if (!match) {
      const regexAlt =
        /}\s*\(\s*(['"][\s\S]+?['"])\s*,\s*(\d+)\s*,\s*(\d+)\s*,\s*(['"][\s\S]+?['"])\.split/i;
...
function extractStreamtapeLink(html: string): string | null {
  try {
    const match =
      html.match(/id=['"]robotlink['"]\s*>([\s\S]*?)<\/div>/i) ||
      html.match(
        /(?:document\.getElementById|_\$)\(['"]robotlink['"]\)\.innerHTML\s*=\s*(['"][\s\S]+?);/i,
      );
    if (match) {
      ...
      for (let term of terms) {
        ...
        const subMatch = term.match(
          /^\s*\(\s*['"]([^'"]*)['"]\s*\)\s*\.\s*substring\s*\(\s*(\d+)\s*\)\s*$/,
        );
        if (subMatch) {
          url += subMatch[1].substring(parseInt(subMatch[2], 10));
          continue;
        }
        const subMatch2 = term.match(
          /^\s*['"]([^'"]*)['"]\s*\.\s*substring\s*\(\s*(\d+)\s*\)\s*$/,
        );
        if (subMatch2) {
...
async function extractDoodstreamLink(
  targetUrl: string,
  html: string,
  ...
  try {
    ...
    try {
      ...
      const isDirectExtension =
        ...
        pathname.includes("playlist.m3u8") ||
        search.includes(".mp4") ||
      ...
      if (
        ...
        return {
          ...
          sources: [
            {
              file: targetUrl,
              label: "سيرفر البث المباشر (HD)",
              type: lowercaseUrl.includes(".m3u8") ? "hls" : "mp4",
            },
          ],
        };
      }
    ...
    for (let depth = 0; depth < 2; depth++) {
      ...
      let iframeSrc =
        $(
          'iframe[src*="embed"], iframe[src*="video"], iframe[src*="player"], iframe[src*="ok.ru"], iframe[src*="govid"], iframe[src*="stream"], iframe[src*="v="], iframe[src*="dood"], iframe[src*="tape"]',
        )
          .first()
          .attr("src") ||
      ...
      if (iframeSrc) {
        ...
        if (
          ...
          try {
            ...
            if (
              nestedLower.includes(".mp4") ||
              nestedLower.includes(".m3u8") ||
              nestedLower.includes(".mkv")
            ) {
            ...
            const nextResp = await fetchWithTimeout(
              iframeSrc,
              { headers, method: "GET" },
              1500,
            );
            if (nextResp.ok) {
              const nextContentType =
                nextResp.headers.get("content-type") || "";
              const nextContentLength =
                nextResp.headers.get("content-length") || "";

              if (isMediaContentType(nextContentType, nextContentLength)) {
                const isHls =
                  nextContentType.toLowerCase().includes("mpegurl") ||
                  iframeSrc.toLowerCase().includes("m3u8");
                return {
                  directUrl: iframeSrc,
                  type: isHls ? "hls" : "direct",
                  sources: [
                    {
                      file: iframeSrc,
                      label: "بث مباشر ذكي من الإطار (HD)",
                      type: isHls ? "hls" : "mp4",
                    },
                  ],
                };
    ...
    if (currentUrl.includes("ok.ru")) {
      const match =
        ...
        html.match(/OK\.Player\.initPlayer\([^,]+,\s*([^)]+)\)/);
      if (match) {
        try {
          ...
          const videos = json?.flashvars?.metadata
            ? JSON.parse(json.flashvars.metadata)?.videos
            : json?.videos;
    ...
    if (currentUrl.includes("streamtape.com") || currentUrl.includes("tape")) {
      ...
      if (direct) {
        sources.push({
          file: direct,
          label: "سيرفر Streamtape المباشر (HD)",
          type: "mp4",
        });
      }
    }
    ...
    if (currentUrl.includes("filemoon")) {
      ...
      if (fmMatch) {
        sources.push({
          file: fmMatch[1],
          label: "سيرفر Filemoon مباشر (HLS)",
          type: "hls",
        });
      }
    }
    ...
    if (
      currentUrl.includes("streamwish") ||
      currentUrl.includes("awish") ||
      currentUrl.includes("strwish") ||
      currentUrl.includes("wish") ||
      currentUrl.includes("swish")
    ) {
      const wishMatch =
        html.match(
    ...
    if (
      currentUrl.includes("mp4upload") ||
      currentUrl.includes("md4") ||
      currentUrl.includes("mdloader")
    ) {
      const mp4Match =
        html.match(
          /player\.src\(\s*\{\s*type\s*:\s*['"]video\/mp4['"]\s*,\s*src\s*:\s*['"](https?:[^'"]+\.mp4[^'"]*)['"]/i,
        ) ||
        html.match(/src\s*:\s*['"](https?:[^'"]+\.mp4[^'"]*)['"]/i) ||
        html.match(/file\s*:\s*['"](https?:[^'"]+\.mp4[^'"]*)['"]/i) ||
        combinedCodeToScan.match(
    ...
    if (currentUrl.includes("sibnet")) {
      ...
      if (sibMatch) {
        const fileUrl = sibMatch[1].startsWith("/")
          ? `https://video.sibnet.ru${sibMatch[1]}`
          : sibMatch[1];
        sources.push({
          file: fileUrl,
          label: "سيرفر Sibnet مباشر (HD)",
          type: "mp4",
        });
      }
    }
    ...
    if (currentUrl.includes("mixdrop")) {
      const mixMatch =
        combinedCodeToScan.match(
          /(?:wurl|remp|delivery)\s*=\s*["']([^"']+)["']/i,
        ) ||
        combinedCodeToScan.match(
          /[^A-Za-z0-9](?:https?:)?\/\/[^\s'"]+?mixdrop[^\s'"]+?\.mp4[^\s'"]*/i,
        );
      if (mixMatch) {
    ...
    if (currentUrl.includes("dood") || currentUrl.includes("d000d")) {
      const direct = await extractDoodstreamLink(
        currentUrl,
        combinedCodeToScan,
        headers,
      );
      if (direct) {
        sources.push({
          file: direct,
          label: "سيرفر Doodstream فائق السرعة",
          type: "mp4",
        });
      }
    }
    ...
    const m3u8Matches: string[] = Array.from(new Set(m3u8MatchesRaw)).filter(
      (url) =>
        !url.includes("google") &&
        !url.includes("facebook") &&
        !url.includes("analytics"),
    );
    const mp4Matches: string[] = Array.from(new Set(mp4MatchesRaw)).filter(
      (url) =>
        !url.includes("google") &&
        !url.includes("facebook") &&
        !url.includes("analytics") &&
      ...
      try {
        ...
        if (decoded.startsWith("http://") || decoded.startsWith("https://")) {
          if (
            ...
            !decoded.includes("facebook")
          ) {
            sources.push({
              file: decoded,
              label: "فك تشفير ذكي (HLS)",
              type: "hls",
            });
          } else if (
  ...
  handleAsync(async (req: Request, res: Response) => {
    try {
      ...
      const requestedSeason = (
        (req.query.season as string) || "auto"
      ).toLowerCase();
      ...
      const currentDay = days.includes(userDay)
        ? userDay
      ...
      if (bypass) {
        ...
        if (cached !== null) {
          if (Array.isArray(cached.top5) && cached.top5.length > 0 && Array.isArray(cached.recentEpisodes) && cached.recentEpisodes.length >= 15) {
            return res.json({ success: true, data: cached });
          } else {
            console.warn(`[ServerCache] Rejecting empty/corrupted or Jikan-fallback dashboard cache for key: ${cacheKey}`);
            await serverCache.delete(cacheKey);
          }
        }
      }
      ...
      if (
        ...
        ["winter", "spring", "summer", "fall"].includes(requestedSeason)
      ) {
      ...
      const [popularJson, seasonJson, recentList, todayScheduleJson] =
        await Promise.all([
          fetchJikan(
          ...
          fetchJikan(
            `https://api.jikan.moe/v4/schedules?filter=${currentDay}&limit=20`,
          ).catch((err) => {
      ...
      // Translate top5 titles to Arabic
      const top5 = await Promise.all(
        (seasonJson.data || []).slice(0, 15).map(async (anime: any) => {
          const rawTitle = anime.title_english || anime.title;
          const arabicTitle = await getArabicTitle(rawTitle);
          return {
      ...
      // Map recent scraped episodes, filtering duplicates to prevent overlapping
      const resolvedRecentList = await Promise.all(
        (recentList || []).map(async (item) => {
          const enTitle = await translateTitleToEnglish(item.title);
          // Scraped item title is already Arabic. No need to double translate or query getArabicTitle!
          const arTitle = item.title;
          const lowerTitle = enTitle.toLowerCase().trim();
          return { item, enTitle, arTitle, lowerTitle };
        })
      );

      const seenRecentTitles = new Set<string>();
      const recentEpisodes: any[] = [];
      resolvedRecentList.forEach(({ item, enTitle, arTitle, lowerTitle }, idx) => {
        if (!seenRecentTitles.has(lowerTitle)) {
          seenRecentTitles.add(lowerTitle);

          const posterUrl = item.imageUrl
            ? item.imageUrl.replace("-thumbnail.", "-poster.")
            : "https://via.placeholder.com/300x400?text=Anime";
          const thumbnailUrl =
            item.imageUrl || "https://via.placeholder.com/300x400?text=Anime";

          recentEpisodes.push({
            _id: `recent-scraped-${idx}`,
            animeId: {
              _id: `search-${item.title}`,
              title: arTitle, // Keep original Arabic title (or translated Arabic) for perfect mobile RTL rendering!
              titleAr: arTitle,
              posterUrl,
              thumbnailUrl,
            },
      ...
      // Translate popular titles to Arabic
      const popular = await Promise.all(
        (popularJson.data || []).map(async (anime: any) => {
          const rawTitle = anime.title_english || anime.title;
          const arabicTitle = await getArabicTitle(rawTitle);
          return {
            _id: anime.mal_id.toString(),
      ...
      // Filter duplicates for current season, translating titles to Arabic
      const seenSeasonIds = new Set<string>();
      const currentSeasonRaw = (seasonJson.data || []).filter((anime: any) => {
      ...
      const currentSeason = await Promise.all(
        currentSeasonRaw.map(async (anime: any) => {
          const rawTitle = anime.title_english || anime.title;
          const arabicTitle = await getArabicTitle(rawTitle);
          return {
            _id: anime.mal_id.toString(),
      ...
      // Filter duplicates for schedule, translating titles to Arabic
      const seenScheduleIds = new Set<string>();
      const scheduleRaw = (todayScheduleJson.data || []).filter((anime: any) => {
        const malId = anime.mal_id.toString();
      ...
      const schedule = await Promise.all(
        scheduleRaw.map(async (anime: any) => {
          const rawTitle = anime.title_english || anime.title;
          const arabicTitle = await getArabicTitle(rawTitle);
          return {
            _id: anime.mal_id.toString(),
      ...
      const containsFallback = (recentList || []).some((item: any) => item.link && item.link.startsWith('/anime/'));
      // Cache the whole dashboard payload for 15 minutes in double-layered cache if it contains recent episodes and top5 data
      if (recentEpisodes.length >= 15 && top5.length > 0 && !containsFallback) {
        await serverCache.set(cacheKey, dashboardData, 15 * 60 * 1000);
      } else if (recentEpisodes.length > 0) {
        // Cache fallback dashboard for only 30 seconds to allow quick recovery when scraper is back online
        await serverCache.set(cacheKey, dashboardData, 30 * 1000);
      }
    ...
    try {
      ...
      if (jikanRes && jikanRes.data) {
        return res.json({
          success: true,
          posterUrl:
            jikanRes.data.images?.jpg?.large_image_url ||
            jikanRes.data.images?.jpg?.image_url,
        });
      }
...
router.get(
  ...
  handleAsync(async (req: Request, res: Response) => {
    try {
      ...
      const picsRes = await fetchJikan(
        `https://api.jikan.moe/v4/anime/${id}/pictures`,
      ).catch((e) => {
        console.log("Pictures fetch handled:", e.message || e);
        return null;
      });
      const pictures =
        picsRes?.data?.map(
          (p: any) =>
            p.large_image_url || p.jpg?.large_image_url || p.image_url,
        ) || [];
    ...
    try {
      ...
      if (id !== "random") {
        ...
        if (
          cached !== null &&
          cached.description &&
          cached.description !== "جاري تحديث قصة وتفاصيل هذا الأنمي قريباً."
        ) {
          // If cached description is in English and has no Arabic characters, translate it on-the-fly!
          const hasEnglishDescription = /[a-zA-Z]{15,}/.test(cached.description) && !/[\u0600-\u06FF]/.test(cached.description);
          if (hasEnglishDescription) {
            console.log(`[Details API] Cached description is in English. Re-translating on-the-fly for ${id}`);
            try {
              const cleanedDesc = cached.description.replace(/\[Written by MAL Rewrite\]/g, "").trim();
              cached.description = await translateToArabic(cleanedDesc);
              await serverCache.set(`anime-details-${id}`, cached, 24 * 60 * 60 * 1000);
            } catch (err) {
              console.warn("On-the-fly translation of cached synopsis failed:", err);
            }
          }
          return res.json({ success: true, data: cached });
        }
      ...
      if (id === "random") {
        ...
        try {
          const randomRes = await fetchJikan(
          ...
          if (randomRes && randomRes.data) {
            ...
            id =
              CLASSIC_POPULAR_IDS[
                Math.floor(Math.random() * CLASSIC_POPULAR_IDS.length)
              ];
          }
        } catch (e) {
          console.log(
            "Failed to fetch random anime from jikan, using fallback classic popular ID",
            e,
          );
          id =
            CLASSIC_POPULAR_IDS[
      ...
      if (/^\d+$/.test(id)) {
        ...
        try {
          const [fullRes, charsRes, relRes, recsRes] = await Promise.all([
            fetchJikan(`https://api.jikan.moe/v4/anime/${id}/full`).catch(
              (e) => {
                console.log("Full anime metadata info:", e.message || e);
                return null;
              },
            ),
            fetchJikan(`https://api.jikan.moe/v4/anime/${id}/characters`).catch(
              (e) => {
                console.log("Characters info:", e.message || e);
                return null;
              },
            ),
            fetchJikan(`https://api.jikan.moe/v4/anime/${id}/relations`).catch(
              (e) => {
                console.log("Relations info:", e.message || e);
                return null;
              },
            ),
            fetchJikan(
              ...
              console.log("Recommendations info:", e.message || e);
              return null;
            }),
          ]);
          ...
          if (jikanDataRes?.data && jikanDataRes.data.title) {
            const searchedId = await scraper.searchAnime(
              jikanDataRes.data.title,
            );
        ...
        try {
          const englishTitle = await translateTitleToEnglish(titleQuery);
          const jikanSearch = await fetchJikan(
            `https://api.jikan.moe/v4/anime?q=${encodeURIComponent(englishTitle || titleQuery)}&limit=1`,
          );
          if (jikanSearch && jikanSearch.data && jikanSearch.data.length > 0) {
            const malId = jikanSearch.data[0].mal_id.toString();
            const [fullRes, charsRes, relRes, recsRes] = await Promise.all([
              fetchJikan(`https://api.jikan.moe/v4/anime/${malId}/full`).catch(
                (e) => {
                  console.warn(e);
                  return null;
                },
            ...
            if (recsRes && recsRes.data) {
              jikanRecommendations = recsRes.data
                .slice(0, 10)
                .map((r: any) => ({
                  id: r.entry.mal_id,
                  title: r.entry.title,
          ...
          console.log(
            "Failed to resolve Jikan metadata for search query details:",
            titleQuery,
            e,
          );
        }
        ...
        if (!searchedId && !jikanDataRes) {
          return res
            ...
            .json({
              success: false,
              message: "لم يتم العثور على الأنمي في انمي ليك",
            });
        }
      }
      ...
      try {
        ...
        console.log(
          `[getAnimeDetails] AnimeLek scraper failed for ${animelekId}`,
          e,
        );
      }
      ...
      if (!data) {
        if (jikanDataRes?.data) {
          data = {
            ...
            posterUrl:
              jikanDataRes.data.images?.jpg?.large_image_url ||
              jikanDataRes.data.images?.jpg?.image_url,
            description: jikanDataRes.data.synopsis || "لا توجد قصة متاحة.",
            episodes: [],
            genres: jikanDataRes.data.genres?.map((g: any) => g.name) || [
              "غير محدد",
            ],
            rating: jikanDataRes.data.score || 0,
            status:
              jikanDataRes.data.status === "Finished Airing"
                ? "مكتمل"
                : jikanDataRes.data.status === "Currently Airing"
                  ? "مستمر"
                  : "قادم",
          ...
          return res
            ...
            .json({
              success: false,
              message: "لم يتم العثور على تفاصيل الأنمي.",
            });
        }
      }
      ...
      if (data && jikanDataRes?.data) {
        data.title =
          jikanDataRes.data.title_english ||
          jikanDataRes.data.title ||
          data.title;
        data.posterUrl =
          jikanDataRes.data.images?.jpg?.large_image_url ||
        ...
        if (!data.rating || data.rating === "?")
          data.rating = jikanDataRes.data.score;
      }
      ...
      if (data && (!data.episodes || data.episodes.length === 0)) {
        ...
        if (numericId) {
          try {
            const jpEpisodesRes = await fetchJikan(
              `https://api.jikan.moe/v4/anime/${numericId}/episodes`,
            );
            if (
              jpEpisodesRes &&
              Array.isArray(jpEpisodesRes.data) &&
              jpEpisodesRes.data.length > 0
            ) {
              const now = new Date();
              const airedEpisodes = jpEpisodesRes.data.filter((ep: any) => {
                if (ep.aired) {
                  try {
              ...
              data.episodes = airedEpisodes.map((ep: any) => ({
                id: `${numericId}-episode-${ep.mal_id}`,
                num: ep.mal_id.toString(),
                title: ep.title
                  ? `الحلقة ${ep.mal_id} - ${ep.title}`
                  : `الحلقة ${ep.mal_id}`,
              ...
              const isOngoing =
                ...
                jikanDataRes?.data?.status === "Currently Airing" ||
                jikanDataRes?.data?.airing === true;
              const count = !isOngoing ? parseInt(data.episodesCount) || 12 : 0;
      ...
      // Ensure accurate sorting and deduplication for the episodes lists with zero gaps
      if (data) {
        if (!data.episodes) data.episodes = [];
        const seenIds = new Set<string>();
        data.episodes = data.episodes.filter((ep: any) => {
          if (!ep || !ep.id) return false;
          const key = ep.id.toString();
          if (seenIds.has(key)) return false;
          seenIds.add(key);
          return true;
        });

        // Sort ascending by episode number to make presentation perfectly ordered and accurate
        data.episodes.sort(
          (a: any, b: any) => (parseInt(a.num) || 0) - (parseInt(b.num) || 0),
        );
      }
      ...
      if (data) {
        ...
        if (requestParamId && requestParamId !== "random") {
          await serverCache.set(
            `anime-details-${requestParamId}`,
            data,
            24 * 60 * 60 * 1000,
          ); // 24 hours cache for ultra-speed
        }
        if (data._id && data._id !== "random" && data._id !== requestParamId) {
          await serverCache.set(
            `anime-details-${data._id}`,
            data,
            24 * 60 * 60 * 1000,
          ); // 24 hours cache for ultra-speed
        }
...
async function smartQueryTranslator(queryStr: string): Promise<string> {
  ...
  if (LOCAL_ANIME_MAP[normalizedQuery]) {
    console.log(
      `[Smart Translation] Local Map Hit: "${trimmed}" -> "${LOCAL_ANIME_MAP[normalizedQuery]}"`,
    );
  ...
  if (LOCAL_ANIME_MAP[trimmed]) {
    console.log(
      `[Smart Translation] Local Map Direct Hit: "${trimmed}" -> "${LOCAL_ANIME_MAP[trimmed]}"`,
    );
  ...
  if (!client) {
    console.warn(
      "GEMINI_API_KEY is not configured for search query translator, returning original query.",
    );
    return trimmed;
  }
...
Respond with ONLY the plain English translated title or character name. Do NOT include any additional labels, quotes, explanation, or markdown.`;
  ...
  for (const model of modelsToTry) {
    try {
      ...
      if (response?.text) {
        ...
        console.log(
          `Smart Search Query translator using ${model}: "${trimmed}" -> "${result}"`,
        );
      ...
      console.warn(
        `Gemini query translator using ${model} failed (trying next fallback):`,
        err.message || err,
      );
    }
  }
  ...
  console.warn(
    "All search query translation models bypassed. Returning original input query.",
  );
  return trimmed;
}
...
router.get(
  ...
  handleAsync(async (req: Request, res: Response) => {
    try {
      ...
      res
        .status(500)
        .json({ success: false, message: "فشل في جلب التصنيفات." });
    }
  }),
);
    ...
    try {
      ...
      if (year && season) {
        ...
        const numberOfDays = Math.floor(
          (Date.now() - oneJan.getTime()) / (24 * 60 * 60 * 1000),
        );
      ...
      res
        .status(500)
        .json({ success: false, message: "فشل في جلب أنميات الموسم." });
    }
  }),
);
...
router.get(
  ...
  handleAsync(async (req: Request, res: Response) => {
    try {
      ...
      res
        .status(500)
        .json({ success: false, message: "فشل في جلب جدول المواعيد." });
    }
  }),
);
...
router.post(
  ...
  handleAsync(async (req: Request, res: Response) => {
    ...
    if (!name) {
      return res
        .status(400)
        .json({ success: false, message: "اسم المهمة مطلوب" });
    }
    const { ScraperQueue } = await import("../services/queueService");
    ...
    res.json({
      success: true,
      message:
        "تمت جدولة عملية الـ Scraping وإضافتها لصف الانتظار للعمل بشكل غير متزامن.",
      jobId,
    });
  }),
);
...
router.post(
  ...
  handleAsync(async (req: Request, res: Response) => {
    ...
    res.json({
      success: true,
      message: "تم تصفية سجل صفوف العمل المكتملة والمعلقة.",
    });
  }),
);
